...
```
В консоли видим что в первом тест-сьюте браузер запустился один раз, а во втором — два раза.
...
class TestMainPage1():
    ...
    def test_guest_should_see_basket_link_on_the_main_page(self, browser):
        ...
        browser.find_element_by_css_selector(".basket-mini .btn-group > a")
```
Есть альтернативный способ вызова teardown кода с помощью встроенной фикстуры **request** и ее метода **addfinalizer**. Можете изучить его сами по документации [PyTest][1].

[1]: https://docs.pytest.org/en/latest/how-to/fixtures.html#adding-finalizers-directly
